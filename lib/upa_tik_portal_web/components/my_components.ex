defmodule UpaTikPortalWeb.Components.MyComponents do
  ...
  def navbar(assigns) do
      ...
      <div class="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between">
        ...
        <div class="flex items-center gap-6">
          ...
          <div class="hidden md:flex items-center gap-2 border-r border-slate-200 pr-6">
            ...
            <.nav_link navigate={~p"/portal/ajukan"} active={@active_tab == :ajukan}>
              Pengajuan
            </.nav_link>
            <.nav_link navigate={~p"/portal/status"} active={@active_tab == :status}>
              Status
            </.nav_link>
            <.nav_link navigate={~p"/portal/keluhan"} active={@active_tab == :keluhan}>
              Lapor Masalah
            </.nav_link>
          </div>
          ...
          <div class="relative">
            ...
            <div
              id="profile-dropdown"
              class="hidden absolute right-0 mt-2 w-48 bg-white rounded-xl shadow-lg z-50 overflow-hidden"
            >
              <!-- ... isi dropdown ... -->
              <div class="px-4 py-3 border-b border-slate-100 bg-slate-50/50">
                ...
                <p class="text-sm font-semibold text-slate-900 truncate">
                  <%!-- <%= inspect(@current_user, pretty: true) %> --%> {@current_user &&
                    @current_user.name}
                </p>
              </div>
    ...
    <main class="max-w-7xl mx-auto mt-4 px-4 pb-20">{render_slot(@inner_block)}</main>
    """
  end
  ...
  defp mobile_sidebar(assigns) do
    ...
    <div
      ...
      class="relative z-50 md:hidden hidden"
      role="dialog"
      aria-modal="true"
    >
      <!-- Backdrop -->
      ...
      >
        ...
        <nav class="flex-1 px-4 py-6 space-y-2">
          <.mobile_nav_link navigate={~p"/portal"} active={@active_tab == :home} icon="hero-home">
            Beranda
          </.mobile_nav_link>
          <.mobile_nav_link
            navigate={~p"/portal/ajukan"}
            active={@active_tab == :ajukan}
            icon="hero-document-plus"
          >
            Pengajuan
          </.mobile_nav_link>
          <.mobile_nav_link
            navigate={~p"/portal/status"}
  ...
  defp mobile_nav_link(assigns) do
    ...
    >
      <.icon name={@icon} class={["w-5 h-5", @active && "text-white", !@active && "text-slate-400"]} /> {render_slot(
        @inner_block
      )}
    </.link>
    """
    ...
    <nav class="border-b border-slate-200 shadow-sm bg-transparent dark:bg-slate-900">
      <div class="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
        ...
        <div class="flex gap-6 items-center">
          ...
          <.nav_link navigate={~p"/admin/pengajuan"} active={@active_tab == :pengajuan}>
            Pengajuan
          </.nav_link>
          <.nav_link navigate={~p"/admin/keluhan"} active={@active_tab == :keluhan}>
            Keluhan
          </.nav_link>
  ...
  defp hide_mobile_sidebar(js \\ %JS{}) do
    ...
    |> JS.transition({"ease-in duration-200", "translate-x-0", "-translate-x-full"},
      to: "#mobile-sidebar-panel"
    )
