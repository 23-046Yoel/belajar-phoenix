defmodule UpaTikPortalWeb.Admin.RequestListLive do
  ...
  defp fetch_requests(socket) do
    ...
    requests = Requests.list_requests_filtered(filters)
  ...
  def render(assigns) do
      ...
      <div class="flex flex-col md:flex-row justify-between items-center bg-white p-8 rounded-[2rem] shadow-xl shadow-slate-200/50 border border-slate-100 gap-6">
        ...
        <div class="flex flex-col gap-4 w-full md:w-auto">
          <div class="flex flex-col sm:flex-row items-center gap-4">
            ...
            <div class="flex bg-slate-50 p-2 rounded-2xl border border-slate-100 gap-1 w-full sm:w-auto overflow-x-auto">
              <%= for {label, val} <- [{"Semua", "all"}, {"Pending", "pending"}, {"Setuju", "disetujui"}, {"Tolak", "ditolak"}] do %>
                <button
                  phx-click="filter"
    ...
    <!-- Date Filters and Export in List View -->
          <div class="flex flex-col sm:flex-row items-center justify-between gap-4 bg-slate-50/50 p-4 rounded-2xl border border-slate-100">
            <form phx-change="filter_dates" class="flex flex-wrap items-center gap-3 w-full sm:w-auto">
              <div class="flex items-center gap-2">
                ...
                <input
                  ...
                  name="start_date"
                  value={@start_date}
                  class="px-3 py-1.5 bg-white border border-slate-200 rounded-lg text-xs font-bold focus:outline-none focus:border-indigo-500"
                />
              </div>
              <div class="flex items-center gap-2">
            ...
            <div class="flex gap-2 w-full sm:w-auto justify-end">
              <a
                href={
                  ~p"/admin/reports/requests?status=#{@filter}&search=#{@search}&start_date=#{@start_date}&end_date=#{@end_date}"
                }
                class="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-xs font-black uppercase tracking-widest transition-all flex items-center gap-1.5 shadow-md shadow-indigo-100"
              >
              ...
              <a
                href={
                  ~p"/admin/reports/requests?status=#{@filter}&search=#{@search}&start_date=#{@start_date}&end_date=#{@end_date}&format=csv"
                }
